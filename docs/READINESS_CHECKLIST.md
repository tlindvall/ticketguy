...
| G4 | A model key for whichever `EXTRACTION_PROVIDER` names (`OPENAI_API_KEY` / `ANTHROPIC_API_KEY`), verified per-token rates in `MODEL_PRICES_USD_PER_MTOKEN`, spend budget; bounded eval run | Owner | Live extraction/drafting; eval report |
