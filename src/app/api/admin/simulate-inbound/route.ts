...
/** Fixture/development-only simulator of the normalized inbound contract. Never claims provider signature verification. */
export async function POST(req: Request) {
  return adminRoute(req, { body: Body }, async ({ body }) => {
    const e = env();
    if (e.APP_MODE !== 'fixture' || e.isProductionLike) return Response.json({ error: 'simulator_disabled_outside_fixture_mode' }, { status: 403 });
